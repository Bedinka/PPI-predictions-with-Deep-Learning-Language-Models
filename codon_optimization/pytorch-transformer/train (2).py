...
def get_ds(config, aa_filepath, mRNA_filepath):

    ds_raw = load_aminoacid_mRNA_data(aa_filepath, mRNA_filepath)
...
def train_model(config, aa_filepath, mRNA_filepath):
    # Define the device
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("Using device:", device)
    ...
    train_dataloader, val_dataloader, tokenizer_src, tokenizer_tgt = get_ds(config, aa_filepath, mRNA_filepath)
    model = get_model(config, tokenizer_src.get_vocab_size(), tokenizer_tgt.get_vocab_size()).to(device)
    # Tensorboard
...
if __name__ == "__main__":
    ...
    params = [
        {
            "aa_filepath": r".\data\chlamydomonas_aa_middle.fa",
            "mrna_filepath": r".\data\chlamydomonas_mrna_middle.fa",
            "num_epochs": 10, "model_folder": "weights_middle", 
            "model_basename": "tmodel_middle", 
            "experiment_name": "runs/tmodel_middle"
            },

        {
            "aa_filepath": r".\data\chlamydomonas_aa_start.fa", 
            "mrna_filepath": r".\data\chlamydomonas_mrna_start.fa", 
            "num_epochs": 5, "model_folder": "weights_start", 
            "model_basename": "tmodel_start", 
            "experiment_name": "runs/tmodel_start"
            },

        {
            "aa_filepath": r".\data\chlamydomonas_aa_end.fa", 
            "mrna_filepath": r".\data\chlamydomonas_mrna_end.fa",
            "num_epochs": 5, "model_folder": "weights_end",
            "model_basename": "tmodel_end",
            "experiment_name": "runs/tmodel_end"
            }
        ]
    ...
    for param in params:
        ...
        aa_filepath = param['aa_filepath']
        mrna_filepath = param['mrna_filepath']
        config['num_epochs'] = param['num_epochs']
        config['model_folder'] = param['model_folder']
        config['model_basename'] = param['model_basename']
        config['experiment_name'] = param['experiment_name']
        config['preload'] = None
        
        print(f'training f{config["model_basename"]}...')
        
        train_model(config, aa_filepath, mrna_filepath)